                ...
                foreach (CustomMaterialList cm in cmList)
                {
                    ...
                    if (dsVendorsListByCategory != null)
                    {
                        ...
                        for (int counter = 0; counter < dsVendorsListByCategory.Tables[0].Rows.Count; counter++)
                        {
                            ...
                            try
                            {
                                CommonFunction.SendEmail(JGConstant.EmailTypes.VendorCategories.ToString(), "", mailId, "J.M. Grove " + jobId + " quote request ", htmlBody, new List<Attachment>(), htmlViews);
                                emailCounter += 1;
                            }
                            catch (Exception ex)
                            {
        ...
        protected bool sendEmailToVendors(List<CustomMaterialList> cmList)
        {
            ...
            try
            {
                ...
                foreach (CustomMaterialList cm in cmList)
                {
                    ...
                    try
                    {
                        CommonFunction.SendEmail(JGConstant.EmailTypes.Vendors.ToString(), "", mailId, emailSubject, htmlBody, objAttachment, htmlViews);
                        emailCounter += 1;
                    }
                    catch (Exception ex)
                    {
        ...
        public bool SendOrderonEmails(string pVendorCategoryIds, StringBuilder pStrMaterialListTable, Int32 pProductCatID)
        {
            ...
            if (dsVendorsListByCategory != null)
            {
                ...
                for (int counter = 0; counter < dsVendorsListByCategory.Tables[0].Rows.Count; counter++)
                {
                    ...
                    try
                    {
                        CommonFunction.SendEmail(JGConstant.EmailTypes.Orders.ToString(), "", mailId, emailSubject, htmlBody, objAttachment, htmlViews);
                        emailCounter += 1;
                    }
                    catch (Exception ex)
                    {
