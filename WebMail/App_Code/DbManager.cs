...
namespace WebMail
{
	...
	public abstract class DbManager : IDisposable
	{
        ...
        public static DataSet GetUserById(int id)
        {
            try
            {
                DataSet ds = new DataSet();
                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["JGPA"].ConnectionString))
                {
                    SqlCommand sqlComm = new SqlCommand("GetUserById", conn);
