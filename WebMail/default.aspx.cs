...
namespace WebMail
{
	...
	public partial class _default : Page
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
            ...
            // Check if query string has userid
            if (!string.IsNullOrEmpty(Request.QueryString["id"]))
            {
                // fetch user via id
                DataSet user = DbManager.GetUserById(Convert.ToInt32(Request.QueryString["id"].ToString()));
                //Request[]
                email.Value = user.Tables[0].Rows[0]["PortalEmail"].ToString();
                pwd.Value = user.Tables[0].Rows[0]["PortalEmailPassword"].ToString();
            }
