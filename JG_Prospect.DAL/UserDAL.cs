...
namespace JG_Prospect.DAL
{
    public class UserDAL
    {
        ...
        public int UpdatePhoneScript(int id, int type, int subType, string title, string desc, int did, string scriptType)
        {
            try
            {
                SqlDatabase database = MSSQLDataBase.Instance.GetDefaultDatabase();
                {
                    DbCommand command = database.GetStoredProcCommand("AddUpdatePhoneScript");
                    command.CommandType = CommandType.StoredProcedure;
                    database.AddInParameter(command, "@Id", DbType.Int32, id);
                    database.AddInParameter(command, "@Type", DbType.Int32, type);
                    database.AddInParameter(command, "@SubType", DbType.Int32, subType);
                    database.AddInParameter(command, "@Title", DbType.String, title);
                    database.AddInParameter(command, "@ScriptType", DbType.String, scriptType);
                    database.AddInParameter(command, "@Script", DbType.String, desc);
                    database.AddInParameter(command, "@DesignationId", DbType.Int32, did);
                    returndata = database.ExecuteDataSet(command);
                    return Convert.ToInt32(returndata.Tables[0].Rows[0]["Id"]);
                }
            }
            catch (Exception ex)
            {
        ...
        public void SaveCallPosition(int callerUserId, int candidateUserId)
        {
            try
            {
                SqlDatabase database = MSSQLDataBase.Instance.GetDefaultDatabase();
                {
                    DbCommand command = database.GetStoredProcCommand("SaveCallPosition");
                    command.CommandType = CommandType.StoredProcedure;
                    database.AddInParameter(command, "@CallerUserId", DbType.Int32, callerUserId);
                    database.AddInParameter(command, "@CandidateUserId", DbType.Int32, candidateUserId);
                    database.ExecuteScalar(command);
                }
            }
            catch (Exception ex)
        ...
        public void UpdatePhoneScriptFAQ(int id, string title, string desc, int did)
        {
            try
            {
                SqlDatabase database = MSSQLDataBase.Instance.GetDefaultDatabase();
        ...
        public List<PhoneScript> GetPhoneScripts(int designationId, int type, int subType)
        {
            try
            {
                ...
                {
                    returndata = new DataSet();
                    DbCommand command = database.GetStoredProcCommand("GetPhoneScriptByDesignationId");
        ...
        public int GetCallPosition(int callerUserId)
        {
            try
            {
                SqlDatabase database = MSSQLDataBase.Instance.GetDefaultDatabase();
                {
                    returndata = new DataSet();
                    DbCommand command = database.GetStoredProcCommand("GetCallPosition");
                    database.AddInParameter(command, "@CallerUserId", DbType.Int32, callerUserId);
                    command.CommandType = CommandType.StoredProcedure;
                    returndata = database.ExecuteDataSet(command);
                    if (returndata != null && returndata.Tables[0] != null && returndata.Tables[0].Rows.Count > 0)
                    {
                        return Convert.ToInt32(returndata.Tables[0].Rows[0]["CandidateUserId"].ToString());
                    }
                    return 0;
                }
            }
